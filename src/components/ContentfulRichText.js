...
function renderTextContent({ value, marks }) {
  ...
  if (marks && marks.length > 0) {
    return (
      <Text
        as="span"
        fontWeight={markMap.bold && "bold"}
        fontStyle={markMap.italic && "italic"}
      >
        {value}
      </Text>
    );
...
function ContentfulParagraph({ content, links, root, children, ...props }) {
  if (content.length === 3) {
    ...
    if (content[1].data.target) {
      ...
      const asset = links?.entries?.inline?.filter(
        (l) => l.sys.id === firstID
      )[0];

      if (asset) {
        return (
          <CodeBlock lang={asset.language} numbers={true}>
            {asset.code}
          </CodeBlock>
        );
      }
...
function mapRichText({
  ...
  if (nodeType === "text") {
    return value
      .split(/(\r\n|\n\r|\r|\n)/g)
      .map((part) => renderTextContent({ value: part, marks }));
  }
  ...
  const nodeTypes = {
    ...
    paragraph: (
      <ContentfulParagraph root={isRootElement} links={links} content={content}>
        {innerContent}
      </ContentfulParagraph>
    ),
    "heading-1": (
...
export default function ContentfulRichText({ json, links, h1Size }) {
  if (!json) return <></>;
  return (
    <StaticContent>{mapRichText({ ...json, links, h1Size })}</StaticContent>
  );
}
